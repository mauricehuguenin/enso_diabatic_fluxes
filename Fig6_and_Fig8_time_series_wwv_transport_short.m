...
dV(1) = dV(1) / (30*24*60*60) / 1e6; % nochmals durch 1 Million um schÃ¶n in Sverdrup zu plotten
plot(dV); hold on;
...
dV = dV / delta_t / 1e6; % nochmals durch 1 Million um schÃ¶n in Sverdrup zu plotten
plot(dV); hold on;
...
for t = 1:24
    GJ(:,:,t) = (mass(:,:,t)) .* wwv_mask_2S_borneo .* 1e-3 ./ 1e6; % umrechnen von [kg sâ?»1] zu [m^3 s^-1]
